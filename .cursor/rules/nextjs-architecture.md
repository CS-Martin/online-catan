...
export class CasinoDiscoveryService {
  static async saveDiscoveredCasinos(
    discoveredData: DiscoverCasino[]
  ): Promise<SaveResult> {
...
export async function POST() {
  try {
    ...
    const result = await CasinoDiscoveryService.saveDiscoveredCasinos(
      discovered.discoverCasino
    );
